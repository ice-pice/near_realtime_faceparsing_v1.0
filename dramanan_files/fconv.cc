...
void *process(void *thread_arg) {
  ...
  for (int f = 0; f < num_features; f++) {
    ...
    /*for (int x = td.BB(1); x < C_dims[1]; x++) {*/
      /*for (int y = 0; y < C_dims[0]; y++) {*/
      double *dst = C + x*C_dims[0] + (int)BB[1];
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
  ...
  const mxArray *mxBB = prhs[4];
  const mwSize *BB_dims = mxGetDimensions(mxBB);
  double *BB = (double *)mxGetPr(mxBB);
  ...
  for (int i = 0; i < len; i++) {
    ...
    if ( td.BB[0] != -1 && td.BB[1] != -1 && td.BB[2] != -1 && td.BB[3] != -1)
    {
	td.BB[0] -= (td.B_dims[1] - 1)/2;
	td.BB[1] -= (td.B_dims[0] - 1)/2;
	td.BB[2] -= (td.B_dims[1] - 1)/2;
	td.BB[3] -= (td.B_dims[0] - 1)/2;
       process((void *)&td);
    }
